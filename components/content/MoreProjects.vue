    ...
    <section class="max-w-prose mx-auto flex flex-row flex-wrap justify-between items-center">
        <ContentList v-slot="{ list }" :path="$route.path.split('/').slice(0, 2).join('/') + '/'" :head="false"
            ...
            <div v-for="article in list" :key="article._path"
                class="w-full prose dark:prose-invert md:my-4 flex overflow-hidden flex-row [&_img]:hover:opacity-70 [&_img]:hover:scale-105 max-w-none isolate">

                <div class="w-1/4 flex items-center">
                    <a class="overflow-hidden rounded-md" :href="`${article._path}`"><img class="m-0 transition-[opacity,transform]" :src="article.thumbnail" alt="" /></a>
                </div>
